...
mysql_select_db($database_sistemacol, $sistemacol);
$query_reporte = sprintf("SELECT a.nombre as nomalum, a.apellido, a.cedula, a.alumno_id, b.no_lista, d.nombre as anio, e.descripcion FROM jos_alumno_info a, jos_alumno_curso b, jos_curso c, jos_anio_nombre d, jos_seccion e WHERE b.curso_id=c.id AND a.alumno_id=b.alumno_id AND c.anio_id=d.id AND c.seccion_id=e.id AND a.cursando=1 AND b.curso_id= %s  ORDER BY a.cedula ASC " , GetSQLValueString($colname_reporte, "int"));
$reporte = mysql_query($query_reporte, $sistemacol) or die(mysql_error());
$row_reporte = mysql_fetch_assoc($reporte);
$totalRows_reporte = mysql_num_rows($reporte);
}
...
<table width="800" style="font-size:9px" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#006600" bordercolor="#000000" bordercolordark="#000000" class="fondo_tabla" id="marco_tabla">
		...
		<td height="26"  style="border-left:1px solid" class="texto_mediano_gris"><div align="center"><span><?php echo $row_reporte['cedula']; ?></span></div></td>
<td style="border-left:1px solid" class="texto_mediano_gris">&nbsp;&nbsp;<?php echo $row_reporte['apellido']; ?></td>
<td style="border-left:1px solid" class="texto_mediano_gris">&nbsp;&nbsp;<?php echo $row_reporte['nomalum']; ?></td>

              
<td style="border-left:1px solid"><div align="center"><span><input type="text" size="2" maxlength="2" name="<?php echo 'no_lista'.$i;?>" id="<? echo 'no_lista'.$i;?>" value="<?php echo $i; ?>" /></span></div> 
...
<tr style="background-color:#fffccc;" height="30" > <td colspan="26" align="right"><span><b>Verifica los Datos y presiona una (1) sola vez click...</b> &nbsp;&nbsp;</span><input name="Actualizar" type="submit" value="Configurar No. Lista">    </td></tr>
