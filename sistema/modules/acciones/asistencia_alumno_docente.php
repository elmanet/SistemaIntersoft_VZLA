...
mysql_select_db($database_sistemacol, $sistemacol);
$query_docente = sprintf("SELECT c.id as mate, d.nombre, f.descripcion, g.nombre as anio, e.id as curso_id  FROM jos_asignatura c, jos_asignatura_nombre d, jos_curso e, jos_seccion f, jos_anio_nombre g WHERE c.asignatura_nombre_id=d.id AND c.curso_id=e.id AND e.seccion_id=f.id AND e.anio_id=g.id AND c.id= %s", GetSQLValueString($colname_docente, "text"));
...
$inasistencia_alumno=$_POST['inasistencia'];
     ...
     $insertSQL = sprintf("INSERT INTO jos_alumno_asistencia (id, alumno_id, asignatura_id, inasistencia, inasistencia_alumno, justificativo, fecha_inasistencia) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
$justificativo="";
  ...
  $insertGoTo = "asistencia_alumno2.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<form action="<?php echo $editFormAction; ?>asignatura_id=<?php echo $_POST['asignatura_id']?>" name="new_form" id="new_form" method="POST" enctype="multipart/form-data" onKeyPress="return disableEnterKey(event)">
    <h3 style="color:red;">IMPORTANTE: Los campos de la siguiente tabla son OBLIGATORIOS!</h3>
    <table style="background-color: #fffccc; border: 1px solid;"><tr>
            <td></td><td></td><td></td>
        </tr>
        <tr>
            <td>
	  ...
	  $row_docente = mysql_fetch_assoc($docente);
	   ...
	   ?>
            ...
            <td>
             <b> Horas de Clase:</b> <input type="text" size="2" maxlength="2" name="inasistencia" id="inasistencia" value="" />
            </td>
            <td>
                <b>Fecha de la Clase: </b><input type="text" size="15" maxlength="20" name="fecha_inasistencia" id="fecha_inasistencia" value="" /> <span style="font-size:9px; font-family: verdana; color: red;"><b>Ej. 2011-04-25</b></span>
            </td>
        </tr>
    <tr>
                  <td colspan="4"><b>Selecciona aqui si hubo asistencia Total: </b><input type="checkbox" name="si" id="si" value="1" /></td></tr></td></tr>
    </table>
<br />
...
<table width="800" style="font-size:9px" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#006600" bordercolor="#000000" bordercolordark="#000000" class="fondo_tabla" id="marco_tabla">
		...
		<td height="26"  style="border-left:1px solid"><div align="center"><span><?php echo $row_reporte['cedula']; ?></span></div></td>
                ...
                <td style="border-left:1px solid"><div align="center"><span><input type="checkbox" name="<?php echo 'valor2'.$i;?>" id="<? echo 'valor2'.$i;?>" value="1" />
                 <input type="text" name="<?php echo 'inasistencia_alumno'.$i;?>" id="<? echo 'inasistencia_alumno'.$i;?>" value="" size="2" /></span></div></td>
