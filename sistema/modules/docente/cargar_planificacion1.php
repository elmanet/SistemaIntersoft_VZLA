...
mysql_select_db($database_sistemacol, $sistemacol);
$query_docente = sprintf("SELECT a.id, a.username, a.password, a.gid, c.id as mate, d.nombre, f.descripcion, g.nombre as anio, e.id as curso_id, b.id as docente_id  FROM jos_users a, jos_docente b, jos_asignatura c, jos_asignatura_nombre d, jos_curso e, jos_seccion f, jos_anio_nombre g WHERE  a.id=b.joomla_id AND b.id=c.docente_id AND c.asignatura_nombre_id=d.id AND c.curso_id=e.id AND e.seccion_id=f.id AND e.anio_id=g.id AND c.orden_asignatura<20 AND a.username = %s ORDER BY c.orden_asignatura ASC", GetSQLValueString($colname_docente, "text"));
...
<table width="760" border="0" cellspacing="0" cellpadding="0">
  <!--DWLayoutTable-->
  <tr>
  <h2>Subir Planificaci&oacute;n</h2>
    <td valign="top"><h3>Paso 1. Seleccione la Asignatura y el tipo de Planificaci&oacute;n</h3>
	<form action="planificacion/Upload3.php" method="POST" name="f1">
	...
	<tr><td class="texto_mediano_gris">
		<input type="radio" name="tipo_plan" value="1">Planificaci&oacute;n Anual
		<br>
		<input type="radio" name="tipo_plan" value="2">Planificaci&oacute;n de Lapso
		<br>
		<input type="radio" name="tipo_plan" value="3">Plan de Evaluaci&oacute;n Mensual
	</td></tr>
	         <tr><td class="texto_mediano_gris"><br /><b>Lapso</b></td></tr>
	<tr><td class="texto_mediano_gris">
		<input type="radio" name="lapso" value="1">1er Lapso
		<br>
		<input type="radio" name="lapso" value="2">2do Lapso
		<br>
		<input type="radio" name="lapso" value="3">3er Lapso
	</td>
	...
	<input type="hidden" name="docente_id" value="<?php echo $row_docente['docente_id']?>"/>
	
	<div style="margin-top:10px;">
	
	<input type="submit" name="buttom" value="Ir al Paso 2 >" style="font-size:18px"/>
	</td></tr>
	</form>
<tr><td>
&nbsp;&nbsp;
