...
mysql_select_db($database_sistemacol, $sistemacol);
$query_planilla = sprintf("SELECT * FROM jos_formato_evaluacion_pr a, jos_alumno_info b WHERE a.lapso=$lapso_curso AND a.alumno_id=b.alumno_id AND a.asignatura_id = %s ORDER BY b.apellido ASC", GetSQLValueString($colname_planilla, "int"));
...
do { 
   $i++;
	...
	// FIN CALCULO
     $insertSQL = sprintf("INSERT INTO jos_formato_evaluacion_pr (id, alumno_id, asignatura_id, confiplanilla_id, lapso, inasistencia, des_cualitativa, def_cuantitativa, def_cualitativa) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                            ...
                            GetSQLValueString($_POST['inasistencia'.$i], "int"),
                            GetSQLValueString($_POST['des_cualitativa'.$i], "text"),
                            GetSQLValueString($_POST['def_cuantitativa'.$i], "int"),
  ...
  $insertGoTo = "fin_carga_notas_pr.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
do { 
   $i++;
	...
	// FIN CALCULO
     $updateSQL = sprintf("update jos_formato_evaluacion_pr SET alumno_id=%s, asignatura_id=%s, confiplanilla_id=%s, lapso=%s, inasistencia=%s, des_cualitativa=%s, def_cuantitativa=%s, def_cualitativa=%s WHERE id=%s",
                            GetSQLValueString($_POST['alumno_id'.$i], "int"),
                            GetSQLValueString($_POST['asignatura_id'.$i], "int"),
                            GetSQLValueString($_POST['confiplanilla_id'.$i], "int"),
                            GetSQLValueString($_POST['lapso'.$i], "int"),
  ...
  $updateGoTo = "fin_carga_notas_pr.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<b>Inasistencias:</b> <input name="<?php echo 'inasistencia'.$i;?>" type="text" id="<? echo 'inasistencia'.$i;?>" value="<?php echo $row_planilla['inasistencia']; ?>" style="width: 30px; height: 15px; font-size:9px;" />&nbsp;&nbsp;
<b>Def. Cuantitativa:</b>&nbsp;&nbsp; <input name="<?php echo 'def_cuantitativa'.$i;?>" type="text" id="<? echo 'def_cuantitativa'.$i;?>" value="<?php echo $row_planilla['def_cuantitativa']; ?>" style="width: 30px; height: 15px; font-size:9px;" />&nbsp;&nbsp;
<b>Def. Cualitativa:</b>&nbsp;&nbsp; <input name="<?php echo 'def_cualitativa'.$i;?>" type="text" id="<? echo 'def_cualitativa'.$i;?>" value="<?php echo $row_planilla['def_cualitativa']; ?>" style="width: 30px; height: 15px; font-size:9px;" /><br /><br />
