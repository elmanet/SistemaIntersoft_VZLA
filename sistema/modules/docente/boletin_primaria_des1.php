...
mysql_select_db($database_sistemacol, $sistemacol);
$query_planilla = sprintf("SELECT * FROM jos_formato_evaluacion_pr_especialistas a, jos_alumno_info b WHERE a.lapso=$lapso_curso AND a.alumno_id=b.alumno_id AND a.asignatura_id = %s ORDER BY b.apellido ASC", GetSQLValueString($colname_planilla, "int"));
...
do { 
   $i++;
	...
	// FIN CALCULO
     $insertSQL = sprintf("INSERT INTO jos_formato_evaluacion_pr_especialistas (id, alumno_id, asignatura_id, lapso, des_cualitativa, def_cualitativa) VALUES (%s, %s, %s, %s, %s, %s)",
...
do { 
   $i++;
	...
	// FIN CALCULO
     $updateSQL = sprintf("update jos_formato_evaluacion_pr_especialistas SET alumno_id=%s, asignatura_id=%s, lapso=%s, des_cualitativa=%s, def_cualitativa=%s WHERE id=%s",
                            GetSQLValueString($_POST['alumno_id'.$i], "int"),
                            GetSQLValueString($_POST['asignatura_id'.$i], "int"),
                            GetSQLValueString($_POST['lapso'.$i], "int"),
                            GetSQLValueString($_POST['des_cualitativa'.$i], "text"),
...
<b>Apreciaci&oacute;n Cualitativa:</b>&nbsp;&nbsp; <input name="<?php echo 'def_cualitativa'.$i;?>" type="text" id="<? echo 'def_cualitativa'.$i;?>" value="<?php echo $row_planilla['def_cualitativa']; ?>" style="width: 30px; height: 15px; font-size:9px;" /><br /><br />
