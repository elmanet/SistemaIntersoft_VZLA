...
mysql_select_db($database_sistemacol, $sistemacol);
$query_asignaturas = sprintf("SELECT * FROM jos_asignatura a, jos_asignatura_nombre b WHERE a.asignatura_nombre_id=b.id AND a.tipo_asignatura='' AND b.condicion=1 AND a.curso_id=%s", GetSQLValueString($colname_asignaturas, "int"));
...
mysql_select_db($database_sistemacol, $sistemacol);
$query_resumen = sprintf("SELECT b.cedula, b.indicador_nacionalidad, a.no_alumno, a.m1, a.m2, a.m3, a.m4, a.m5, a.m6, a.m7, a.m8, a.m9, a.m10, a.m11, a.m12, a.m13, a.m14, a.ept1, a.ept2, a.ept3, a.ept4, a.momento FROM jos_cdc_resumen_pendiente a, jos_alumno_info b WHERE a.alumno_id=b.alumno_id AND b.cursando=1  AND a.no_alumno>=$hojaini AND a.no_alumno<=$hojafin AND curso_id=%s AND tipo_resumen=%s AND tipo_cedula=%s AND a.momento=%s ORDER BY a.no_alumno ASC", GetSQLValueString($colname_resumen, "int"), GetSQLValueString($tr_resumen, "int"), GetSQLValueString($tc_resumen, "int"), GetSQLValueString($momento_resumen, "int"));
...
mysql_select_db($database_sistemacol, $sistemacol);
$query_observacion = sprintf("SELECT * FROM jos_cdc_resumen_observaciones WHERE curso_id=%s AND tipo_resumen=%s AND tipo_cedula=%s ", GetSQLValueString($colname_observacion, "int"), GetSQLValueString($tr_observacion, "int"), GetSQLValueString($tc_observacion, "int"));
...
mysql_select_db($database_sistemacol, $sistemacol);
$query_resumen_alumno = sprintf("SELECT * FROM jos_cdc_resumen_pendiente a, jos_alumno_info b WHERE a.alumno_id=b.alumno_id AND b.cursando=1 AND a.no_alumno>=$hojaini AND a.no_alumno<=$hojafin AND a.curso_id=%s AND a.tipo_resumen=%s AND a.tipo_cedula=%s AND a.momento=%s ORDER BY a.no_alumno ASC", GetSQLValueString($colname_resumen_alumno, "int"), GetSQLValueString($tr_resumen_alumno, "int"), GetSQLValueString($tc_resumen_alumno, "int"), GetSQLValueString($momento_resumen_alumno, "int"));
...
<div id="logo_me">
<div id="logo">
<img src="../../images/logo_me_cer.jpg" border="0" align="absmiddle"> 
</div>
...
<table class="tabla">
	<tr>
		...
		<td colspan="14" align="center"><b>Calificaciones de las Asignaturas</b></td>
...
if($row_planestudio['cod']=="32011"){ ?>
<td class="cuadrito">
ET
</td>
<?php $i=2; }?>

<?php if($row_planestudio['cod']=="31018"){ 
$i=1; 
}?>
...
<?php  $i++; } while ($i <= $totalmates); ?>
	...
	<tr>
		<td><?php echo $no;?></td>
		<td style="text-align:left;">&nbsp;&nbsp;<?php echo $row_resumen['indicador_nacionalidad']."-".$row_resumen['cedula'];?></td>
...
?></td>
		...
		<td>*</td>
		<td><?php if($row_resumen['ept1']==1){echo "X";}else{echo "*";}?></td>
		<td><?php if($row_resumen['ept2']==1){echo "X";}else{echo "*";}?></td>
		<td><?php if($row_resumen['ept3']==1){echo "X";}else{echo "*";}?></td>
		<td><?php if($row_resumen['ept4']==1){echo "X";}else{echo "*";}?></td>
	...
	<?php $no++; } while($no <= $tr); }?>
		<td rowspan="5" colspan="2"><b>III. TOTALES</b></td>


	</tr>
	<tr>
<?php // ASIGNATURAS INSCRITAS ?>
		<td class="cuadrito"><?php echo $sm1;?></td>
		<td class="cuadrito"><?php echo $sm2;?></td>
		<td class="cuadrito"><?php echo $sm3;?></td>
		<td class="cuadrito"><?php echo $sm4;?></td>
		<td class="cuadrito"><?php echo $sm5;?></td>
		<td class="cuadrito"><?php echo $sm6;?></td>
		<td class="cuadrito"><?php if($sm7==0){echo "*";}else{echo $sm7;}?></td>
		<td class="cuadrito"><?php if($sm8==0){echo "*";}else{echo $sm8;}?></td>


		<td class="cuadrito"><?php if($valorm9!=""){ if($sm9==0){echo "*";}else{echo $sm9;} } if(($valorm14!="") and ($valorm9==NULL) and ($valorm8!="")){ echo $sm14; } if(($valorm8!="") and ($valorm9==NULL) and ($valorm14==NULL)){ echo "*"; } if(($valorm8==NULL) and ($valorm9==NULL) and ($valorm14==NULL)){ echo "*"; } if(($valorm8==NULL) and ($valorm9==NULL) and ($valorm14!="")){ echo "*"; }?></td>


		<td class="cuadrito"><?php if($valorm10!=""){ if($sm10==0){echo "*";}else{echo $sm10;} } if(($valorm14!="") and ($valorm10==NULL) and ($valorm9!="")){ echo $sm14; } if(($valorm9!="") and ($valorm10==NULL) and ($valorm14==NULL)){ echo "*"; } if(($valorm9==NULL) and ($valorm10==NULL) and ($valorm14==NULL)){ echo "*"; } if(($valorm9==NULL) and ($valorm10==NULL) and ($valorm14!="")){ echo "*"; }?></td>


		<td class="cuadrito"><?php if($valorm11!=""){ if($sm11==0){echo "*";}else{echo $sm11;} } if(($valorm14!="") and ($valorm11==NULL) and ($valorm10!="")){ echo $sm14; } if(($valorm10!="") and ($valorm11==NULL) and ($valorm14==NULL)){ echo "*"; } if(($valorm10==NULL) and ($valorm11==NULL) and ($valorm14==NULL)){ echo "*"; } if(($valorm10==NULL) and ($valorm11==NULL) and ($valorm14!="")){ echo "*"; }?></td>
...
<?php // INASISTENCIAS ?>		
	...
	<?php $no=1; do { ?>
	<tr>
		<td><?php echo $row_resumen_alumno['no_alumno'];?></td>
		<td colspan="3" style="text-align:left;">&nbsp;&nbsp;<?php echo $row_resumen_alumno['apellido'];?></td>
		...
		<td style="text-align:left;">&nbsp;&nbsp;<?php echo $row_resumen_alumno['sexo'];?></td>
			<?php
	...
	<?php } while ($row_resumen_alumno = mysql_fetch_assoc($resumen_alumno)); ?>
	...
	<?php $no++; } while($no <= $tr);} ?>
		...
		<td class="cuadrito">&nbsp;</td>
		<td colspan="3"><b>Asignaturas</b></td>
		...
		<tr>
			<td style="height:0.4cm;border-width: 0 0 1px 0;" ><?php echo $row_seccion['orden_resumen'];?></td>
		</tr>
		<tr>
		...
		<tr>
			<td style="height:0.42cm;border-width: 0 0 1px 0;" ><?php echo $totalRows_resumen; ?></td>
		</tr>
		<tr>
		...
		<tr>
			<td style="height:0.4cm;border-width: 0 0 0px 0px;" >&nbsp;</td>
		</tr>
	</table>
	</tr>
	...
	<tr>
		<td class="cuadrito"><?php echo $no; ?></td>
		<td colspan="3" style="text-align:left; ">&nbsp;<?php echo $row_verdocentes['nombre_mate']; ?></td>
	...
	<?php if($no<15){ $no=$totalRows_verdocentes+1; $tr=14; do { ?>
	<tr>
		<td class="cuadrito"><?php echo $no; ?></td>
		<td colspan="3">*************</td>
		<td colspan="7">*************</td>
	...
	<tr>
		<td colspan="21" style="text-align:left; font-size:9pt;">&nbsp;<b>V. Programas cursados en Educaci&oacute;n para el Trabajo/ horas-Estudiantes Semanales de C/uno</b></td>
	</tr>
	...
	<tr>
		...
		<td colspan="9" style="text-align:left; font-size:8pt;">&nbsp;<?php echo $row_mate_ept3['descripcion'];?></td>
	...
	<tr>
		...
		<td colspan="9" style="text-align:left; font-size:8pt;">&nbsp;<?php echo $row_mate_ept4['descripcion'];?></td>
...
<div id="observaciones">
	<div class="lineas">
	<b>IV. Observaciones:</b>&nbsp;&nbsp;&nbsp;<?php echo $row_observacion['observacion']; ?> 
	</div>
	<div class="lineas">
	</div>
	<div class="lineas">
