...
if ($confi=="bol01"){ // inicio if
     ...
     $updateSQL = sprintf("update jos_formato_evaluacion_bol01 SET def=%s WHERE id=%s",
                         
                            GetSQLValueString($_POST['def'.$i], "double"),
...
if ($confi=="bol02"){ // inicio if
     ...
     $updateSQL = sprintf("update jos_formato_evaluacion_bol02 SET def=%s WHERE id=%s",
...
if ($confi=="nor01"){ // inicio if
     ...
     $updateSQL = sprintf("update jos_formato_evaluacion_nor01 SET def=%s WHERE id=%s",
...
if ($confi=="nor02"){ // inicio if
     ...
     $updateSQL = sprintf("update jos_formato_evaluacion_nor02 SET def=%s WHERE id=%s",
  ...
  $updateGoTo = "cargar_notas_definitivas2.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<!--INFORMACION DE LA CONSULTA  -->
 <td valign="top" align="left" class="texto_mediano_gris" ><h2>Definitivas de <?php echo $_POST['lapso']." lapso de ".$row_reporte['mate']." de ".$row_reporte['anio']." ".$row_reporte['descripcion'];?></h2>
...
<table width="700" style="font-size:9px" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#006600" bordercolor="#000000" bordercolordark="#000000" class="fondo_tabla" id="marco_tabla">
		...
		<td height="26"  style="border-left:1px solid"><div align="center"><span><?php echo $row_planilla['cedula']; ?></span></div></td>
                ...
                <td style="border-left:1px solid"><div align="center"><span><input name="<?php echo 'def'.$i;?>" type="text" id="<? echo 'def'.$i;?>" value="<?php echo $row_planilla['def']; ?>" style="width: 30px; height: 15px; font-size:9px;" /></span></div></td>
