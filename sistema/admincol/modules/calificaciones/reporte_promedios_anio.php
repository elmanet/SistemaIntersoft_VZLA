...
if (isset($_GET['id'])) {
  $colname_asignatura = $_GET['id'];
}
mysql_select_db($database_sistemacol, $sistemacol);
$query_asignatura = sprintf("SELECT * FROM jos_anio_nombre WHERE id = %s", GetSQLValueString($colname_asignatura, "text"));
...
if ($confi=="nor01"){
$query_mate15 = sprintf("SELECT a.cedula, a.nombre, a.apellido, ROUND(AVG(c.def),2) as def FROM jos_alumno_info a, jos_alumno_curso b, jos_formato_evaluacion_nor01 c, jos_anio_nombre d, jos_curso e WHERE a.alumno_id=c.alumno_id AND a.alumno_id=b.alumno_id AND b.curso_id=e.id AND e.anio_id = d.id AND d.id = %s AND c.lapso=$lap GROUP BY a.alumno_id ORDER BY def DESC", GetSQLValueString($colname_mate15, "text"));
$mate15 = mysql_query($query_mate15, $sistemacol) or die(mysql_error());
$row_mate15 = mysql_fetch_assoc($mate15);
$totalRows_mate15 = mysql_num_rows($mate15);
}
if ($confi=="nor02"){
$query_mate15 = sprintf("SELECT a.cedula, a.nombre, a.apellido, ROUND(AVG(c.def),2) as def FROM jos_alumno_info a, jos_alumno_curso b, jos_formato_evaluacion_nor02 c, jos_anio_nombre d, jos_curso e WHERE a.alumno_id=c.alumno_id AND a.alumno_id=b.alumno_id AND b.curso_id=e.id AND e.anio_id = d.id AND d.id = %s AND c.lapso=$lap GROUP BY a.alumno_id ORDER BY def DESC", GetSQLValueString($colname_mate15, "text"));
$mate15 = mysql_query($query_mate15, $sistemacol) or die(mysql_error());
$row_mate15 = mysql_fetch_assoc($mate15);
$totalRows_mate15 = mysql_num_rows($mate15);
}
...
<table width="800"><tr>
		...
		<td align="center" width="800" colspan="2">
			<h1>REPORTE DE PROMEDIOS DE <?php echo $row_lapso['descripcion']; ?></h1>
			<h3>Estudiantes de <?php echo $row_asignatura['nombre']; ?></h3>
		</td>
		</tr>
</table>
