...
$colname_mate_eli = "-1";
if (isset($_GET['mate_ept_id'])) {
  $colname_mate_eli = $_GET['mate_ept_id'];
}
mysql_select_db($database_sistemacol, $sistemacol);
$query_mate_eli = sprintf("SELECT * FROM jos_cdc_pensum_asignaturas_ept WHERE id=%s", GetSQLValueString($colname_mate_eli, "text"));
...
mysql_select_db($database_sistemacol, $sistemacol);
$query_mate = sprintf("SELECT a.id, a.nombre_asignatura_ept, c.cod, b.nombre_anio, a.creado  FROM jos_cdc_pensum_asignaturas_ept a, jos_cdc_pensum_anios b, jos_cdc_planestudio c WHERE a.anio_id=b.id AND b.planestudio_id=c.id ORDER BY a.creado DESC");
...
<table width="850" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	<h2><img src="../../images/png/apply_f2.png" border="0" align="absmiddle">&nbsp;&nbsp;Listado de Asignaturas de Educaci&oacute;n para el Trabajo</h2>
...
<a href="pensum_materias_ept_accion.php"><img src="../../images/icons/nuevo.png" border="0" align="absmiddle">&nbsp;Cargar Asignaturas EPT</a>
</div>
...
<tr><td>
<?php if(($_GET['mate_ept_id']>0) and ($totalRows_mate_eli==0)){ ?>
...
<table style="font-size:10px; font-family:verdana;" width="90%" BORDER="1" BORDERCOLOR="#fffccc" CELLSPACING="0" >
	...
	do { ?>
	<tr >
		<td align="center">&nbsp;<?php if($pageNum_mate==1){ $newlista=$maxRows_mate+$lista; echo $newlista; $lista ++;   } if ($pageNum_mate==0){ echo $lista; $lista ++;} ?>&nbsp;</td>
...
&nbsp;&nbsp;
	...
	<tr>
		<td >
		<?php if ($pageNum_mate > 0) { // Show if not first page ?>
